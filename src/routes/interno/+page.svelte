<script lang="ts">
	import DataTablePendentes from '$lib/components/ui/data-table-v-interno/data-table.svelte';
...
{:then LeadsInternos}
	<Tabs.Root value="pendentes" class="relative h-full w-full px-3 md:px-10">
		...
		<Tabs.Content value="pendentes"><DataTablePendentes {LeadsInternos} /></Tabs.Content>
	</Tabs.Root>
{:catch error}
	<p>{error.message}</p>
{/await}
