<script lang="ts">
	...
	// Enhanced status configuration with better dark theme colors
	const statusConfig = {
		Pendente: {
			label: 'Pendente',
		...
		Pago: {
			label: 'Pago',
			badgeColor: 'bg-violet-600 hover:bg-violet-700 border-violet-500/50',
			borderColor: 'border-violet-500/30',
			bgColor: 'bg-violet-950/30',
			textColor: 'text-violet-400',
			ringColor: 'ring-violet-400/30',
			iconColor: 'text-violet-400',
			hoverBorderColor: 'hover:border-violet-400/40',
			hoverShadowColor: 'hover:shadow-violet-500/10',
			...
			{#if paginatedLeads.length === 0}
				<div class="flex h-[60vh] flex-col items-center justify-center gap-6">
					...
					<div class="text-center">
						<p class="mb-2 text-xl font-semibold text-zinc-200">Nenhum lead encontrado</p>
					...
					{#each paginatedLeads as lead}
						...
						>
									...
									<section class="flex flex-col gap-2">
										...
										<div class="flex flex-col gap-3">
											...
											>
												...
												>
													{#if lead.cpf}
														<IdCardIcon class="size-4 text-zinc-400" />
													{:else if lead.cnpj}
														<BuildingIcon class="size-4 text-zinc-400" />
													{:else}
														<IdCardIcon class="size-4 text-zinc-400" />
													{/if}
												</div>
												<div class="min-w-0 flex-1">
													<div
														class="mb-1 text-xs font-medium tracking-wide text-zinc-500 uppercase"
													>
														{lead.cpf ? 'CPF' : lead.cnpj ? 'CNPJ' : 'Documento'}
													</div>
													<div class="truncate text-sm font-semibold text-white">
														{#if lead.cpf}
														...
														{:else}
															<span class="text-zinc-400">Não informado</span>
														{/if}
													</div>
												</div>
											</div>
							...
							>
								...
								<!-- Action Buttons -->
								<div
									class="flex w-full items-center justify-center gap-3 {status === 'Cancelado'
										? 'flex-col'
										: 'flex-row'}"
								>
									{#if status === 'Cancelado'}
									...
									<Dropdown
										{lead}
										{cargo}
										hoverBorderColor={statusConfig[status].hoverBorderColor}
									/>
								</div>
							</footer>
						</article>
